            ...
            <div class="card">
                        ...
                        <div class="col">
                            <div class="row">
                                @if($customer->CustomerStatus==1)
                                    <span class="badge badge-soft-success bold menu-arrow">{{ trans('global.active') }}</span>
                                @else
                                    <span class="badge badge-soft-danger bold menu-arrow">{{ trans('global.closed') }}</span>
                                @endif
                                &nbsp;&nbsp;
                                ...
                                @endif
                                &nbsp;&nbsp;
                                <h4 class="card-title">{{ trans('global.account') }} &nbsp; &#x3a; &nbsp;
                                    {{ $customer->acc_main }} {{ ($customer->acc_sub == 0) ? '' : $customer->acc_sub }}</h4>
                                @if(isset($customer->BillToCustomerID))
                                    <p class="text-muted mb-0">{{ trans('global.accountMain') }} &nbsp; &#x3a; &nbsp;{{ $customer->billingCustomer->CustomerName }}</p>
                                @endif
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <ul class="nav-border nav nav-pills mb-0" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" data-toggle="tab" href="#notes" role="tab" aria-selected="true">{{ trans('global.notes') }}</a>
                        </li>
                    </ul>
